...
- You no longer need to worry about high-severity dependency vulnerabilities; the dashboard build configuration has also been updated for Vite 6 compatibility.
- You no longer experience login lockout persistence after a successful password reset.
- You no longer encounter duplicate key handling issues or negative limit clamping in bulk insert APIs.
- Your PII (personally identifiable information) is now masked from server logs, and emails are redacted.
- You should no longer see the 'ns does not exist' error during your project's first collection creation.
- You now benefit from cryptographically secure `crypto.randomInt()` in place of `Math.random()` for all OTP generation.
</Update>
